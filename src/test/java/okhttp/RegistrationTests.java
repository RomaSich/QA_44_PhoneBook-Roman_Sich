...
public class RegistrationTests implements BaseApi {
    ...
    public void registrationPositiveTest() {
        UserDto user = new UserDto(generateEmail(10), "7206@Rom");
    ...
    public void registrationNegativeTest_ExRes400() throws IOException {
        ...
        ErrorMessageDto errorMessage = GSON.fromJson(response.body().string(), ErrorMessageDto.class);
        softAssert.assertEquals(response.code(), 400,"response code");
        softAssert.assertEquals(errorMessage.getStatus(),400,"status");
        softAssert.assertEquals(errorMessage.getError(),"Bad request","error");
        softAssert.assertTrue(errorMessage.getMessage().toString().contains("must be a well-formed email address"),"message valid");
